...
int main (int argc, char* argv[])
{
  ...
  long correct[10], incorrect[10];
  long max, genre_tag;

  FILE *myfp = fopen("result.txt", "w+");
        /*int i,j;
        int num_files;
        int num_tracks;
...
for(i=0;i<10;i++)
  ...
  while((!feof(docfl)) && fgets(line,(int)lld,docfl)) {
	...
	fprintf(predfl,"%.8g:+1 %.8g:-1\n",dist,-dist);
      ...
      if(doc_label>0) correct[i]++; else incorrect[i]++;
...
/*        0.01 secs, the timer was underflowing.                       */
  ...
  if((!no_accuracy) && (verbosity>=1)) {
    fprintf(myfp,"Accuracy on test set: %.2f%% (%ld correct, %ld incorrect, %ld total)\n",(float)(correct)*100.0/totdoc,correct,incorrect,totdoc);
